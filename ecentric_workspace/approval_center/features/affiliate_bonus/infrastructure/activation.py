...
"""Split activation for Affiliate Bonus: UAT-enable (process Active, card kept
...
from ecentric_workspace.approval_center.features.affiliate_bonus.infrastructure.setup import validate_affiliate_bonus_v1
...
def enable_affiliate_bonus_uat(dry_run=1, apply=0, commit=0):
    ...
    if not v["ok"]:
        report["result"] = ("BLOCKED (validation failed - nothing changed). Blockers: "
                            ...
                            + ". Tip: run setup_affiliate_bonus_v1(apply=1) first if the process is missing.")
        return report
    if not dry:
        frappe.db.set_value("EC Approval Process", PROCESS, "status", "Active")
        try:
            frappe.get_doc("EC Approval Process", PROCESS).add_comment(
                "Comment", _("Affiliate Bonus UAT enabled (process Active; catalog card kept inactive)."))
        except Exception:
            pass
    ...
    if not dry:
        ...
        try:
            frappe.get_doc("EC Approval Type", TYPE).add_comment(
                "Comment", _("Affiliate Bonus published (catalog card Active)."))
        except Exception:
            pass
