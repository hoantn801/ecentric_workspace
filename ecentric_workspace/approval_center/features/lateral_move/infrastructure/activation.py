...
"""Split activation for Employee Lateral Move: UAT-enable (process Active, card kept
...
from ecentric_workspace.approval_center.features.lateral_move.infrastructure.setup import validate_lateral_move_v1

TYPE = "LATERAL_MOVE"
PROCESS = "LATERAL_MOVE-V1"
ROUTE = "approvals/lateral-move"


def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run Employee Lateral Move activation."), frappe.PermissionError)


@frappe.whitelist()
def enable_lateral_move_uat(dry_run=1, apply=0, commit=0):
    """Process Active; catalog card kept INACTIVE (direct-route UAT)."""
    _require_sm()
    dry = is_dry_run(dry_run, apply, commit)
    v = validate_lateral_move_v1()
    ...
    if not v["ok"]:
        report["result"] = ("BLOCKED (validation failed - nothing changed). Blockers: "
                            ...
                            + ". Tip: run setup_lateral_move_v1(apply=1) first if the process is missing.")
        return report
    if not dry:
        frappe.db.set_value("EC Approval Process", PROCESS, "status", "Active")
        try:
            frappe.get_doc("EC Approval Process", PROCESS).add_comment(
                "Comment", _("Employee Lateral Move UAT enabled (process Active; catalog card kept inactive)."))
        except Exception:
            pass
...
@frappe.whitelist()
def publish_lateral_move_after_uat(dry_run=1, apply=0, commit=0):
    ...
    blockers = [c["check"] for c in v.get("checks", []) if not c.get("ok")]
    if not active:
        blockers = blockers + ["process not Active (run enable_lateral_move_uat(apply=1) first)"]
    ...
    if not dry:
        ...
        try:
            frappe.get_doc("EC Approval Type", TYPE).add_comment(
                "Comment", _("Employee Lateral Move published (catalog card Active)."))
        except Exception:
            pass
