...
"""Split activation for Late in - Early out: UAT-enable (process Active, card kept INACTIVE for direct-route
...
from ecentric_workspace.approval_center.features.late_early_out.infrastructure.setup import validate_late_early_out_v1

TYPE = "LATE_EARLY_OUT"
PROCESS = "LATE_EARLY_OUT-V1"
ROUTE = "approvals/late-in-early-out"


def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run Late in - Early out activation."), frappe.PermissionError)


@frappe.whitelist()
def enable_late_early_out_uat(dry_run=1, apply=0, commit=0):
    """Process Active; catalog card kept INACTIVE (direct-route UAT)."""
    _require_sm()
    dry = is_dry_run(dry_run, apply, commit)
    v = validate_late_early_out_v1()
    ...
    if not v["ok"]:
        report["result"] = ("BLOCKED (validation failed). Blockers: " + (", ".join(blockers) or "unknown")
                            + ". Tip: run setup_late_early_out_v1(apply=1) first if the process is missing.")
        return report
    if not dry:
        frappe.db.set_value("EC Approval Process", PROCESS, "status", "Active")
        try:
            frappe.get_doc("EC Approval Process", PROCESS).add_comment(
                "Comment", _("Late in - Early out UAT enabled (process Active; catalog card kept inactive)."))
        except Exception:
            pass
...
@frappe.whitelist()
def publish_late_early_out_after_uat(dry_run=1, apply=0, commit=0):
    ...
    blockers = [c["check"] for c in v.get("checks", []) if not c.get("ok")]
    if not active:
        blockers = blockers + ["process not Active (run enable_late_early_out_uat(apply=1) first)"]
    ...
    if not dry:
        ...
        try:
            frappe.get_doc("EC Approval Type", TYPE).add_comment(
                "Comment", _("Late in - Early out published (catalog card Active)."))
        except Exception:
            pass
