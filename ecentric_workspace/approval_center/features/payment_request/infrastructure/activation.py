...
"""Split activation for Payment Request: UAT-enable (process Active, card kept
...
from ecentric_workspace.approval_center.features.payment_request.infrastructure.setup import validate_payment_request_v1
...
def enable_payment_request_uat(dry_run=1, apply=0, commit=0):
    ...
    if not v["ok"]:
        report["result"] = ("BLOCKED (validation failed - nothing changed). Blockers: "
                            ...
                            + ". Tip: run setup_payment_request_v1(apply=1) first if the process is missing.")
        return report
    if not dry:
        frappe.db.set_value("EC Approval Process", PROCESS, "status", "Active")
        try:
            frappe.get_doc("EC Approval Process", PROCESS).add_comment(
                "Comment", _("Payment Request UAT enabled (process Active; catalog card kept inactive)."))
        except Exception:
            pass
...
@frappe.whitelist()
def publish_payment_request_after_uat(dry_run=1, apply=0, commit=0):
    ...
    if not dry:
        ...
        try:
            frappe.get_doc("EC Approval Type", TYPE).add_comment(
                "Comment", _("Payment Request published (catalog card Active)."))
        except Exception:
            pass
