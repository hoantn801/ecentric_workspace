...
"""Idempotent, System-Manager-only setup for EMPLOYEE_REFERRAL-V1 (Draft): three User
levels Careers Review -> CEO Review (each Any One). Identities are
...
PROCESS_CODE = "EMPLOYEE_REFERRAL-V1"
...
DEFAULT_CAREERS = [role_ref(CNB_ROLE)]
DEFAULT_CEO = ["lam.nguyen@ecentric.vn"]
_LEVELS = [(1, "Careers Review", DEFAULT_CAREERS), (2, "CEO Review", DEFAULT_CEO)]


def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run Employee Referral setup."), frappe.PermissionError)
...
@frappe.whitelist()
def setup_employee_referral_v1(careers=None, ceo=None, dry_run=1, apply=0):
    _require_sm()
    dry = int(apply or 0) != 1
    rep = {"mode": "dry_run" if dry else "apply", "planned": [], "errors": [], "warnings": [], "notes": [], "result": None}
    users = {1: _parse(careers, DEFAULT_CAREERS), 2: _parse(ceo, DEFAULT_CEO)}
    _validate("Careers Reviewer", users[1], rep)
...
def _upsert(users):
    ...
    proc.title = "Employee Referral V1"; proc.approval_type = APPROVAL_TYPE
...
@frappe.whitelist()
def validate_employee_referral_v1():
