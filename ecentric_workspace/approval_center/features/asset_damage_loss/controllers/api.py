...
"""Stable whitelisted compatibility API for Asset Damage/Loss requests."""
import frappe
...
_DEFINITION = get_definition("ASSET_DAMAGE_LOSS")
_FACADE = APPROVAL_FACADE
_caps = _FACADE.capabilities
BIZ, APPROVAL_TYPE = _DEFINITION.business_doctype, _DEFINITION.code
MAX_PAGE, OPEN = _DEFINITION.max_page_length, _caps.OPEN_STATUSES
TERMINAL = ("Approved", "Rejected", "Cancelled")
_EDITABLE_DRAFT, _STATUS_LABEL = _DEFINITION.editable_fields, dict(_DEFINITION.status_labels)
...
@frappe.whitelist()
def get_bootstrap(): return _FACADE.bootstrap(_DEFINITION)

@frappe.whitelist()
def get_form_options(): return _DEFINITION.options_provider()
...
@frappe.whitelist()
def get_detail(name): return _FACADE.detail(_DEFINITION, name)

get_request_detail = get_detail

@frappe.whitelist(methods=["POST"])
def save_draft(name=None, payload=None): return _FACADE.save_draft(_DEFINITION, name, payload)

@frappe.whitelist(methods=["POST"])
def submit_request(name): return _FACADE.submit(_DEFINITION, name)

def _resolve_req(name): return _FACADE.resolve_request(_DEFINITION, name)

@frappe.whitelist(methods=["POST"])
def approve(name, comment=None): return _FACADE.approve(_DEFINITION, name, comment)

@frappe.whitelist(methods=["POST"])
def reject(name, comment=None): return _FACADE.reject(_DEFINITION, name, comment)

@frappe.whitelist(methods=["POST"])
def request_information(name, comment=None): return _FACADE.request_information(_DEFINITION, name, comment)

@frappe.whitelist(methods=["POST"])
def resubmit(name, payload=None): return _FACADE.resubmit(_DEFINITION, name, payload)

@frappe.whitelist(methods=["POST"])
def cancel(name, reason=None): return _FACADE.cancel(_DEFINITION, name, reason)
