...
"""Idempotent, System-Manager-only setup for HR_ACTIVITY-V1 (Draft): three User
levels HR Manager Review -> HOF Review -> CEO Review (each Any One). Identities are
...
DEFAULT_HR_MANAGER = [role_ref(CNB_ROLE)]
DEFAULT_HOF = ["phuong.nguyen1@ecentric.vn"]
DEFAULT_CEO = ["lam.nguyen@ecentric.vn"]
_LEVELS = [(1, "HR Manager Review", DEFAULT_HR_MANAGER), (2, "HOF Review", DEFAULT_HOF), (3, "CEO Review", DEFAULT_CEO)]


def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run HR Activity setup."), frappe.PermissionError)
...
@frappe.whitelist()
def setup_hr_activity_v1(hr_manager=None, hof=None, ceo=None, dry_run=1, apply=0):
    _require_sm()
    dry = int(apply or 0) != 1
    rep = {"mode": "dry_run" if dry else "apply", "planned": [], "errors": [], "warnings": [], "notes": [], "result": None}
    users = {1: _parse(hr_manager, DEFAULT_HR_MANAGER), 2: _parse(hof, DEFAULT_HOF), 3: _parse(ceo, DEFAULT_CEO)}
    _validate("HR Manager", users[1], rep)
    _validate("HOF", users[2], rep)
    _validate("CEO", users[3], rep)
...
def _upsert(users):
    ...
    proc.title = "HR Activity V1"; proc.approval_type = APPROVAL_TYPE
...
def validate_hr_activity_v1():
    ...
    if proc:
        ...
        for l in levels:
            parts = frappe.get_all("EC Approval Participant", filters={"parent": l.name, "participant_purpose": "Approver"},
