...
def claim_fulfillment(name, user=None):
    ...
    engine.ensure_sole_todo(BUSINESS_DT, name, user, _("System Request fulfillment queue"),
...
def complete_fulfillment(name, user=None, payload=None):
    ...
    engine.notify([doc.requested_by, doc.fulfillment_owner],
                  _("System Request da hoan tat: {0}").format(engine.request_label(BUSINESS_DT, name)), BUSINESS_DT, name)
