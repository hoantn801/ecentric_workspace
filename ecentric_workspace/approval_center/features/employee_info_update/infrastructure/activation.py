...
"""Split activation for Employee information update: UAT-enable (process Active, card kept
...
from ecentric_workspace.approval_center.features.employee_info_update.infrastructure.setup import validate_employee_info_update_v1

TYPE = "EMPLOYEE_INFO_UPDATE"
PROCESS = "EMPLOYEE_INFO_UPDATE-V1"
...
def enable_employee_info_update_uat(dry_run=1, apply=0, commit=0):
    ...
    if not v["ok"]:
        report["result"] = ("BLOCKED (validation failed - nothing changed). Blockers: "
                            ...
                            + ". Tip: run setup_employee_info_update_v1(apply=1) first if the process is missing.")
        return report
    if not dry:
        frappe.db.set_value("EC Approval Process", PROCESS, "status", "Active")
        try:
            frappe.get_doc("EC Approval Process", PROCESS).add_comment(
                "Comment", _("Employee information update UAT enabled (process Active; catalog card kept Coming Soon)."))
        except Exception:
            pass
    ...
    if not dry:
        ...
        try:
            frappe.get_doc("EC Approval Type", TYPE).add_comment(
                "Comment", _("Employee information update published (catalog card Active)."))
        except Exception:
            pass
