...
"""Idempotent, System-Manager-only setup for COMPENSATION_LEAVE-V1 (Draft): a single level
'Direct Manager Review' (Requester Manager; no configured approvers - resolved at runtime from
Employee.reports_to, blocked at submit if unresolved). No fulfillment; no SLA. dry-run default;
apply=1 required. Never overwrites an Active process."""
import frappe
from frappe import _

PROCESS_CODE = "COMPENSATION_LEAVE-V1"
APPROVAL_TYPE = "COMPENSATION_LEAVE"


def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run Compensation Leave setup."), frappe.PermissionError)


@frappe.whitelist()
def setup_compensation_leave_v1(dry_run=1, apply=0):
    _require_sm()
...
def _upsert():
    ...
    proc.title = "Compensation Leave V1"
...
@frappe.whitelist()
def validate_compensation_leave_v1():
    ...
    if proc:
        ...
        if levels:
            parts = frappe.get_all("EC Approval Participant",
                                   ...
                                   pluck="source_type")
            c("Requester Manager" in parts, "L1 source is Requester Manager")
