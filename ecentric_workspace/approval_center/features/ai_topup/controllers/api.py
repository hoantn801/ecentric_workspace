...
"""Specialized AI Topup application service exposed through a stable API wrapper.
    ...
    for r in rows:
        ...
        biz = frappe.db.get_value(BIZ, req.reference_name,
                                  ["name", "request_title", "ai_tool", "account_mode", "account_email",
                                   "proposed_account_email", "requested_amount", "department", "creation"], as_dict=True)
        if biz:
...
def submit_request(name):
    ...
    from ecentric_workspace.approval_center.features.ai_topup.application import service as svc
