...
"""Split activation for Livestream supplies request: UAT-enable (process Active, card kept
Coming Soon for direct-route UAT) vs public publish (card Active). System-Manager
...
from ecentric_workspace.approval_center.features.livestream_supplies.infrastructure.setup import validate_livestream_supplies_v1

TYPE = "LIVESTREAM_SUPPLIES"
PROCESS = "LIVESTREAM_SUPPLIES-V1"
ROUTE = "approvals/livestream-supplies"


def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run Livestream supplies request activation."), frappe.PermissionError)


@frappe.whitelist()
def enable_livestream_supplies_uat(dry_run=1, apply=0, commit=0):
    """Process Active; catalog card kept Coming Soon (direct-route UAT)."""
    _require_sm()
    dry = is_dry_run(dry_run, apply, commit)
    v = validate_livestream_supplies_v1()
    ...
    if not v["ok"]:
        report["result"] = ("BLOCKED (validation failed - nothing changed). Blockers: "
                            ...
                            + ". Tip: run setup_livestream_supplies_v1(apply=1) first if the process is missing.")
        return report
    if not dry:
        frappe.db.set_value("EC Approval Process", PROCESS, "status", "Active")
        try:
            frappe.get_doc("EC Approval Process", PROCESS).add_comment(
                "Comment", _("Livestream supplies request UAT enabled (process Active; catalog card kept Coming Soon)."))
        except Exception:
            pass
...
@frappe.whitelist()
def publish_livestream_supplies_after_uat(dry_run=1, apply=0, commit=0):
    ...
    blockers = [c["check"] for c in v.get("checks", []) if not c.get("ok")]
    if not active:
        blockers = blockers + ["process not Active (run enable_livestream_supplies_uat(apply=1) first)"]
    ...
    if not dry:
        ...
        try:
            frappe.get_doc("EC Approval Type", TYPE).add_comment(
                "Comment", _("Livestream supplies request published (catalog card Active)."))
        except Exception:
            pass
