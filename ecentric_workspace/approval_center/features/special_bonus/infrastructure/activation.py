...
"""Split activation for Special Bonus: UAT-enable (process Active, card kept
...
from ecentric_workspace.approval_center.features.special_bonus.infrastructure.setup import validate_special_bonus_v1

TYPE = "SPECIAL_BONUS"
PROCESS = "SPECIAL_BONUS-V1"
ROUTE = "approvals/special-bonus"


def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run Special Bonus activation."), frappe.PermissionError)


@frappe.whitelist()
def enable_special_bonus_uat(dry_run=1, apply=0, commit=0):
    """Process Active; catalog card kept INACTIVE (direct-route UAT)."""
    _require_sm()
    dry = is_dry_run(dry_run, apply, commit)
    v = validate_special_bonus_v1()
    ...
    if not v["ok"]:
        report["result"] = ("BLOCKED (validation failed - nothing changed). Blockers: "
                            ...
                            + ". Tip: run setup_special_bonus_v1(apply=1) first if the process is missing.")
        return report
    if not dry:
        frappe.db.set_value("EC Approval Process", PROCESS, "status", "Active")
        try:
            frappe.get_doc("EC Approval Process", PROCESS).add_comment(
                "Comment", _("Special Bonus UAT enabled (process Active; catalog card kept inactive)."))
        except Exception:
            pass
...
@frappe.whitelist()
def publish_special_bonus_after_uat(dry_run=1, apply=0, commit=0):
    ...
    blockers = [c["check"] for c in v.get("checks", []) if not c.get("ok")]
    if not active:
        blockers = blockers + ["process not Active (run enable_special_bonus_uat(apply=1) first)"]
    ...
    if not dry:
        ...
        try:
            frappe.get_doc("EC Approval Type", TYPE).add_comment(
                "Comment", _("Special Bonus published (catalog card Active)."))
        except Exception:
            pass
