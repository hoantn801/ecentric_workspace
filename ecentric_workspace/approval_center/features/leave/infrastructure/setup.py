...
"""Idempotent, System-Manager-only setup for LEAVE_REQUEST-V1 (Draft): a single level
...
PROCESS_CODE = "LEAVE_REQUEST-V1"
APPROVAL_TYPE = "LEAVE_REQUEST"


def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run Leave setup."), frappe.PermissionError)


@frappe.whitelist()
def setup_leave_v1(dry_run=1, apply=0):
    _require_sm()
