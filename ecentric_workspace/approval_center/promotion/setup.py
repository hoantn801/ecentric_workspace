...
def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run Promotion setup."), frappe.PermissionError)
...
@frappe.whitelist()
def setup_promotion_v1(cnb=None, hof=None, ceo=None, dry_run=1, apply=0):
    _require_sm()
...
def _upsert(users):
    ...
    proc.title = "Promotion Request V1"
