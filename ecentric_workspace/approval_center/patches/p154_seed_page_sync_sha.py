...
def execute():
    ...
    try:
        ...
        frappe.log_error(frappe.get_traceback(), "p154 seed sha FAILED (liet ke feature)")
        return
    for feat in feats:
        ...
        try:
            ...
            failed.append(feat)
            frappe.log_error(frappe.get_traceback(), "p154 seed sha: %s" % feat)
    frappe.log_error(
        "p154 gieo %d trang | bo qua %d | loi %d\nGIEO: %s\nBO QUA: %s\nLOI: %s"
        % (len(seeded), len(skipped), len(failed),
           ", ".join(seeded), ", ".join(skipped), ", ".join(failed)),
        "p154 seed page sync sha")
