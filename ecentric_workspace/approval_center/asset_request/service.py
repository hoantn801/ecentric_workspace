...
BUSINESS_DT = "EC Asset Request"
APPROVAL_TYPE = "ASSET_REQUEST"

MSG_NO_MANAGER = ("Ban chua co Quan ly truc tiep trong he thong. "
                  "Vui long lien he HR/Admin de cap nhat truoc khi gui yeu cau.")
...
MATERIAL_FIELDS = ["request_type", "asset_type", "asset_type_other", "purpose_of_request",
                   "purpose_other", "quantity", "specifications", "justification", "requested_needed_date"]
REQUIRED_AT_SUBMIT = ["request_title", "request_type", "asset_type", "purpose_of_request",
                      "quantity", "specifications", "justification"]
...
def submit(name):
    ...
    if doc.purpose_of_request == "Other" and not (doc.purpose_other or "").strip():
        frappe.throw(_("Vui long nhap muc dich khac khi chon 'Other'."))
    if doc.quantity is not None and int(doc.quantity) <= 0:
        frappe.throw(_("So luong phai lon hon 0."))
    mgr = _manager_user(user)
    if not mgr:
...
def claim_fulfillment(name, user=None):
    ...
    frappe.db.sql(
        """update `tabEC Asset Request` set fulfillment_owner=%s, fulfillment_status='In Progress'
...
def complete_fulfillment(name, user=None, payload=None):
    ...
    if "asset_handover_note" in data:
        doc.asset_handover_note = data.get("asset_handover_note")
