...
**Pinned build:** `pdfjs-dist@4.10.38` **legacy ESM** build (`legacy/build/pdf.js` +
`pdf.worker.js`). This version is **not** affected by CVE-2024-4367 / GHSA-wgrm-67xf-hhpq
...
eval-based shim), sets `GlobalWorkerOptions.workerSrc` to the local `pdf.worker.js`, and
calls `getDocument({ ..., isEvalSupported: false })` as defence in depth.

Committed under `ecentric_workspace/public/vendor/pdfjs/`:
`pdf.js`, `pdf.worker.js`, `LICENSE` (Apache-2.0, upstream, unmodified), and
