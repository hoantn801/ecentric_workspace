...
def _html():
    ...
    with open(os.path.join(base, "frontend", "document_request.main_section.html"), encoding="utf-8") as fh:
        return fh.read()
...
def sync(html=None):
    ...
    frappe.logger("approval_center").info("document_request page_sync: %s Web Page /%s" % (action, ROUTE))
