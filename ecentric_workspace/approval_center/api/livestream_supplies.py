...
"""Permission-safe read + write API for Livestream supplies request (Batch 7). Fixed-participant
...
BIZ = "EC Livestream Supplies Request"
APPROVAL_TYPE = "LIVESTREAM_SUPPLIES"
MAX_PAGE = 50
OPEN = ("Pending", "Information Required")
TERMINAL = ("Approved", "Rejected", "Cancelled")

_EDITABLE_DRAFT = ("supplies", "request_type", "quantity", "justification", "start_date", "end_date", "request_attachment", "department", "company")
...
_LIST_COLS = ['supplies', 'request_type', 'quantity', 'start_date', 'end_date']
...
def get_form_options():
    return {'request_type_options': ['Request supplies', 'Return supplies']}
...
def save_draft(name=None, payload=None):
    ...
    from ecentric_workspace.approval_center.livestream_supplies import service as _svc
...
def submit_request(name):
    from ecentric_workspace.approval_center.livestream_supplies import service as svc
...
def approve(name, comment=None):
    ...
    if not (comment or "").strip():
        frappe.throw(_("Vui long nhap nhan xet khi duyet yeu cau nay."))
