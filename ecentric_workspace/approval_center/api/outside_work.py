...
"""Permission-safe read + write API for Outside Work (form #2). Mirrors AI Topup
...
BIZ = "EC Outside Work Request"
APPROVAL_TYPE = "OUTSIDE_WORK"
MAX_PAGE = 50
OPEN = ("Pending", "Information Required")
TERMINAL = ("Approved", "Rejected", "Cancelled")

_EDITABLE_DRAFT = ("request_title", "work_type", "start_date", "end_date", "duration_days",
                   "remarks", "request_attachment", "department", "company")
...
def get_form_options():
    return {"work_types": ["Key live", "Campaign", "Business trip", "Other"]}


@frappe.whitelist()
def check_overlap(start_date=None, end_date=None, exclude=None):
    """Non-blocking overlap warning: count of the requester's other non-terminal
    Outside Work requests that overlap [start_date, end_date]."""
    from ecentric_workspace.approval_center.outside_work import service as svc
    return {"count": svc.overlap_count(frappe.session.user, start_date, end_date, exclude=exclude)}
...
def list_my_requests(filters=None, start=0, page_length=20):
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "work_type", "start_date", "end_date",
...
def list_need_my_approval(section="pending"):
    ...
    for r in rows:
        ...
        biz = frappe.db.get_value(BIZ, req.reference_name,
                                  ["name", "request_title", "work_type", "start_date", "end_date",
                                   "duration_days", "department", "creation"], as_dict=True)
        if biz:
