...
"""Permission-safe read + write API for Special Bonus. Mirrors AI Topup
...
BIZ = "EC Special Bonus Request"
APPROVAL_TYPE = "SPECIAL_BONUS"
MAX_PAGE = 50
OPEN = ("Pending", "Information Required")
TERMINAL = ("Approved", "Rejected", "Cancelled")

_EDITABLE_DRAFT = ("request_title", "department", "project_name", "reasons", "total_bonus",
...
def list_my_requests(filters=None, start=0, page_length=20):
    ...
    if f.get("project_name"):
        flt["project_name"] = f["project_name"]
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "department", "project_name", "total_bonus",
...
def list_need_my_approval(section="pending"):
    ...
    for r in rows:
        ...
        biz = frappe.db.get_value(BIZ, req.reference_name,
                                  ["name", "request_title", "project_name", "total_bonus",
...
def submit_request(name):
    from ecentric_workspace.approval_center.special_bonus import service as svc
