...
"""Permission-safe read + write API for Daily Target (form #5). Mirrors AI Topup
...
BIZ = "EC Daily Target Request"
APPROVAL_TYPE = "DAILY_TARGET"
MAX_PAGE = 50
OPEN = ("Pending", "Information Required")
TERMINAL = ("Approved", "Rejected", "Cancelled")

_EDITABLE_DRAFT = ("request_title", "request_scope", "brand", "channels", "channel_other",
                   "target_month", "target_setting_type", "justification", "request_attachment",
...
def _active_level_count():
    return 1   # both Daily Target processes have a single approval level
...
def get_form_options():
    return {
        "scopes": ["Project level", "Consolidated / Total"],
...
def list_my_requests(filters=None, start=0, page_length=20):
    ...
    if f.get("request_scope"):
        flt["request_scope"] = f["request_scope"]
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "request_scope", "brand", "target_month",
                                  "target_setting_type", "approval_request", "creation", "modified"],
...
def list_need_my_approval(section="pending"):
    ...
    for r in rows:
        ...
        biz = frappe.db.get_value(BIZ, req.reference_name,
                                  ["name", "request_title", "request_scope", "brand", "target_month",
                                   "target_setting_type", "department", "creation"], as_dict=True)
        if biz:
...
def submit_request(name):
    from ecentric_workspace.approval_center.daily_target import service as svc
