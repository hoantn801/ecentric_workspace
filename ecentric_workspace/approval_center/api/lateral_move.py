...
"""Permission-safe read + write API for Employee Lateral Move. Mirrors AI Topup
...
BIZ = "EC Lateral Move Request"
APPROVAL_TYPE = "LATERAL_MOVE"
MAX_PAGE = 50
OPEN = ("Pending", "Information Required")
TERMINAL = ("Approved", "Rejected", "Cancelled")

_EDITABLE_DRAFT = ("request_title", "new_position", "new_department", "new_line_manager",
                   "transfer_reason", "start_date", "company")
...
def get_form_options():
    return {"new_departments": ["E-commerce Operation", "Service", "Business Development", "Product",
                                "Finance and Accounting", "Operations", "Data & System", "Human Resources"]}
...
def list_my_requests(filters=None, start=0, page_length=20):
    ...
    if f.get("new_department"):
        flt["new_department"] = f["new_department"]
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "new_position", "new_department",
                                  "start_date", "approval_request", "creation", "modified"],
...
def list_need_my_approval(section="pending"):
    ...
    for r in rows:
        ...
        biz = frappe.db.get_value(BIZ, req.reference_name,
                                  ["name", "request_title", "new_position", "new_department",
                                   "start_date", "department", "creation"], as_dict=True)
        if biz:
...
def submit_request(name):
    from ecentric_workspace.approval_center.lateral_move import service as svc
