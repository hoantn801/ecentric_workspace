...
"""Permission-safe read + write API for Budget Setting Request (Batch 8). Sequential 4-level chain:
...
BIZ = "EC Budget Setting Request"
APPROVAL_TYPE = "BUDGET_SETTING"
MAX_PAGE = 50
OPEN = ("Pending", "Information Required")

_EDITABLE_DRAFT = ("budget_period_type", "period_start", "department", "approved_budget_current_period",
...
_REQUIRED = ("budget_period_type", "period_start", "department", "forecast_justification",
             "has_financial_risks", "request_attachment")
_AMOUNTS = ("approved_budget_current_period", "actual_spending_current_period", "forecast_budget_next_period")
...
def gen_title(doc):
    ...
    ps = doc.get("period_start")
    d = getdate(ps) if ps else None
    if doc.get("budget_period_type") == "Monthly":
        tag = ("%04d-%02d" % (d.year, d.month)) if d else "?"
        return ("Budget Setting - Monthly - %s - %s" % (dept, tag))[:180]
    tag = ("%04d" % d.year) if d else "?"
    return ("Budget Setting - Annual - %s - %s" % (dept, tag))[:180]
...
def get_form_options():
    return {"departments": _department_options(), "budget_period_types": BUDGET_PERIOD_TYPES,
            "yes_no": YES_NO}
...
def list_my_requests(filters=None, start=0, page_length=20):
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "budget_period_type", "period_start", "department",
    ...
    for r in rows:
        ...
        biz = frappe.db.get_value(BIZ, req.reference_name,
                                  ["name", "request_title", "budget_period_type", "period_start", "department",
                                   "forecast_budget_next_period", "creation"], as_dict=True)
        if biz:
...
def _validate_for_submit(doc):
    ...
    if doc.budget_period_type not in BUDGET_PERIOD_TYPES:
        frappe.throw(_("Loại kỳ ngân sách không hợp lệ."))
    for f in _AMOUNTS:
        try:
            if float(doc.get(f)) < 0:
                frappe.throw(_("Các giá trị ngân sách không thể là số âm."))
        except (TypeError, ValueError):
            frappe.throw(_("Các giá trị ngân sách phải là số."))
    d = getdate(doc.period_start)
    if doc.budget_period_type == "Annual" and not (d.month == 1 and d.day == 1):
        frappe.throw(_("Với kỳ Năm (Annual), Ngày bắt đầu phải là ngày 1 tháng 1 (01/01)."))
    if doc.budget_period_type == "Monthly" and d.day != 1:
        frappe.throw(_("Với kỳ Tháng (Monthly), Ngày bắt đầu phải là ngày 1 của tháng."))
    if doc.has_financial_risks == "Yes" and not (doc.financial_risk_details or "").strip():
        frappe.throw(_("Vui lòng mô tả rủi ro tài chính khi chọn 'Yes'."))
