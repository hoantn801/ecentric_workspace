...
BIZ = "EC Compensation Leave Request"
APPROVAL_TYPE = "COMPENSATION_LEAVE"
MAX_PAGE = 50
OPEN = ("Pending", "Information Required")
TERMINAL = ("Approved", "Rejected", "Cancelled")

_EDITABLE_DRAFT = ("overtime_start_date", "overtime_end_date", "overtime_duration_days",
...
def list_my_requests(filters=None, start=0, page_length=20):
    ...
    if f.get("cl_start_date"):
        flt["cl_start_date"] = f["cl_start_date"]
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "cl_start_date", "cl_end_date",
                                  "cl_duration_days", "approval_request", "creation", "modified"],
...
def list_need_my_approval(section="pending"):
    ...
    for r in rows:
        ...
        biz = frappe.db.get_value(BIZ, req.reference_name,
                                  ["name", "request_title", "cl_start_date", "cl_end_date",
                                   "cl_duration_days", "department", "creation"], as_dict=True)
        if biz:
...
def save_draft(name=None, payload=None):
    ...
    from ecentric_workspace.approval_center.compensation_leave import service as _svc
...
def submit_request(name):
    from ecentric_workspace.approval_center.compensation_leave import service as svc
