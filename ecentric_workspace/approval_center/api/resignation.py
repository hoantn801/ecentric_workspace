...
"""Permission-safe read + write API for Resignation Request. Mirrors AI Topup
...
_EDITABLE_DRAFT = ("request_title", "resignation_for", "employee_email", "personal_email",
                   "last_working_day", "resignation_reason", "workplace_environment_rating",
                   "benefit_policy_rating", "corporate_culture_rating", "recommend_to_friend",
                   "final_message", "department", "company")
...
_STATUS_LABEL = {"Draft": "Nhap", "Pending": "Dang phe duyet", "Information Required": "Can bo sung",
                 ...
                 "Assigned": "Cho HR xu ly", "In Progress": "HR dang xu ly", "Completed": "Hoan tat"}
...
RATING_OPTIONS = ["5 (Very satisfied)", "4 (Satisfied)", "3 (Neutral)",
...
def list_my_requests(filters=None, start=0, page_length=20):
    ...
    if f.get("resignation_reason"):
        flt["resignation_reason"] = f["resignation_reason"]
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "resignation_for", "resignation_reason",
                                  "last_working_day", "fulfillment_status", "approval_request",
                                  "creation", "modified"],
...
def list_fulfillment_queue(section="unclaimed"):
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "requested_by", "resignation_for",
                                  "resignation_reason", "last_working_day",
                                  "fulfillment_status", "fulfillment_owner", "fulfillment_due_at"],
                          order_by="last_working_day asc", limit_page_length=200)
    return {"rows": rows}
...
def get_detail(name):
    ...
    ff = {"status": biz.fulfillment_status, "owner": biz.fulfillment_owner,
          ...
          "summary": biz.fulfillment_summary, "completed_attachment": biz.completed_attachment}
    return {
...
def submit_request(name):
    from ecentric_workspace.approval_center.resignation import service as svc
