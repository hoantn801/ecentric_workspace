...
"""Permission-safe read + write API for Livestream Sample (form #10). Mirrors AI Topup
...
BIZ = "EC Livestream Sample Request"
APPROVAL_TYPE = "LIVESTREAM_SAMPLE"
MAX_PAGE = 50
OPEN = ("Pending", "Information Required")
TERMINAL = ("Approved", "Rejected", "Cancelled")

_EDITABLE_DRAFT = ("request_title", "brand", "sample_detail", "estimated_arrival_time",
...
def list_my_requests(filters=None, start=0, page_length=20):
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "brand", "estimated_arrival_time",
...
def list_need_my_approval(section="pending"):
    ...
    for r in rows:
        ...
        biz = frappe.db.get_value(BIZ, req.reference_name,
                                  ["name", "request_title", "brand", "estimated_arrival_time", "department", "creation"], as_dict=True)
        if biz:
...
def submit_request(name):
    from ecentric_workspace.approval_center.livestream_sample import service as svc
