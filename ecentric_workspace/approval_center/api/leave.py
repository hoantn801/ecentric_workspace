...
def get_form_options():
    return {"leave_types": ["Annual", "Sick", "Errand", "Maternity", "Paternity",
...
def list_my_requests(filters=None, start=0, page_length=20):
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "leave_type", "start_date", "end_date",
...
def list_need_my_approval(section="pending"):
    ...
    for r in rows:
        ...
        biz = frappe.db.get_value(BIZ, req.reference_name,
                                  ["name", "request_title", "leave_type", "start_date", "end_date",
...
def save_draft(name=None, payload=None):
    ...
    from ecentric_workspace.approval_center.leave import service as _svc
...
def submit_request(name):
    from ecentric_workspace.approval_center.leave import service as svc
