...
"""Permission-safe read + write API for Data Request (form #3). Mirrors AI Topup
...
BIZ = "EC Document Request"
APPROVAL_TYPE = "DOCUMENT_REQUEST"
MAX_PAGE = 50
OPEN = ("Pending", "Information Required")
TERMINAL = ("Approved", "Rejected", "Cancelled")

_EDITABLE_DRAFT = ("request_title", "request_type", "document_name", "owner_department", "detail",
...
def get_form_options():
    # Owner is a searchable Department dropdown sourced from the Department DocType (not hardcoded).
    depts = frappe.get_all("Department", filters={"disabled": 0}, fields=["name", "department_name"],
                           order_by="department_name asc", limit_page_length=0)
    return {
        "request_types": ["Create", "Modify", "Recall"],
        "departments": [{"value": d.name, "label": d.department_name or d.name} for d in depts],
    }
...
def search_departments(query=None):
    flt = {"disabled": 0}
    if query:
        flt["department_name"] = ["like", "%%%s%%" % query]
    depts = frappe.get_all("Department", filters=flt, fields=["name", "department_name"],
                           order_by="department_name asc", limit_page_length=20)
    return {"rows": [{"value": d.name, "label": d.department_name or d.name} for d in depts]}
...
def list_my_requests(filters=None, start=0, page_length=20):
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "request_type", "document_name", "owner_department",
                                  "expected_response_date", "fulfillment_status", "approval_request",
                                  "creation", "modified"],
    ...
    for r in rows:
        ...
        biz = frappe.db.get_value(BIZ, req.reference_name,
                                  ["name", "request_title", "request_type", "document_name", "owner_department",
                                   "expected_response_date", "department", "creation"], as_dict=True)
        if biz:
...
def list_fulfillment_queue(section="unclaimed"):
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "requested_by", "request_type", "document_name",
...
def get_detail(name):
    ...
    ff = {"status": biz.fulfillment_status, "owner": biz.fulfillment_owner,
          ...
          "summary": biz.fulfillment_summary, "document_link": biz.document_link,
...
def submit_request(name):
    from ecentric_workspace.approval_center.document_request import service as svc
