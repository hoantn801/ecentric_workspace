...
_EDITABLE_DRAFT = ("request_title", "request_type", "detailed_description", "expected_resolution_date",
...
_STATUS_LABEL = {"Draft": "Nhap", "Pending": "Dang phe duyet", "Information Required": "Can bo sung",
                 ...
                 "Assigned": "Cho Data xu ly", "In Progress": "Data dang xu ly", "Completed": "Hoan tat"}
...
def get_form_options():
    return {
        "request_types": ["Data accuracy, visualization, retrieval", "Client onboarding",
                          "Client offboarding", "Historical data crawling", "New BI report",
                          "Data training", "Access", "Other"],
        "urgencies": ["U0: as soon as possible", "U1: within next 24 hours",
                      "U2: within next 3 days", "U3: non-urgent / nice to have"],
...
def list_my_requests(filters=None, start=0, page_length=20):
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "request_type", "urgency", "importance",
                                  "expected_resolution_date", "fulfillment_status", "approval_request",
                                  "creation", "modified"],
...
def list_need_my_approval(section="pending"):
    ...
    for r in rows:
        ...
        biz = frappe.db.get_value(BIZ, req.reference_name,
                                  ["name", "request_title", "request_type", "urgency", "importance",
                                   "expected_resolution_date", "department", "creation"], as_dict=True)
        if biz:
...
def list_fulfillment_queue(section="unclaimed"):
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "requested_by", "request_type", "urgency",
                                  "importance", "expected_resolution_date", "fulfillment_status",
                                  "fulfillment_owner", "fulfillment_due_at"],
                          order_by="expected_resolution_date asc", limit_page_length=200)
    return {"rows": rows}
...
def get_detail(name):
    ...
    ff = {"status": biz.fulfillment_status, "owner": biz.fulfillment_owner,
          ...
          "output_attachment": biz.output_attachment}
    return {
...
def submit_request(name):
    from ecentric_workspace.approval_center.data_request import service as svc
