...
"""Permission-safe read + write API for Affiliate Bonus Request (Batch 8). Sequential 4-level chain:
...
BIZ = "EC Affiliate Bonus Request"
APPROVAL_TYPE = "AFFILIATE_BONUS_REQUEST"
MAX_PAGE = 50
OPEN = ("Pending", "Information Required")

_EDITABLE_DRAFT = ("service_month", "detail", "total_amount", "budget", "request_attachment",
                   "department", "company")
_REQUIRED = ("service_month", "detail", "request_attachment")
...
def gen_title(doc):
    sm = doc.get("service_month")
    d = getdate(sm) if sm else None
    mtag = ("%04d-%02d" % (d.year, d.month)) if d else "?"
    amt = doc.get("total_amount")
    amt = ("%.0f" % float(amt)) if amt not in (None, "") else "?"
    return ("Affiliate Bonus - %s - %s" % (mtag, amt))[:180]
...
def list_my_requests(filters=None, start=0, page_length=20):
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "service_month", "total_amount", "budget",
...
def list_need_my_approval(section="pending"):
    ...
    for r in rows:
        ...
        biz = frappe.db.get_value(BIZ, req.reference_name,
                                  ["name", "request_title", "service_month", "total_amount", "budget",
...
def _validate_for_submit(doc):
    ...
    if doc.budget is None:
        missing.append("budget")
    if missing:
        frappe.throw(_("Vui lòng nhập đầy đủ các trường bắt buộc (bao gồm tệp đính kèm) trước khi gửi."))
    try:
        if float(doc.total_amount) <= 0:
            frappe.throw(_("Tổng số tiền phải lớn hơn 0."))
    except (TypeError, ValueError):
        frappe.throw(_("Tổng số tiền phải là số."))
    try:
        if float(doc.budget) < 0:
            frappe.throw(_("Ngân sách không thể là số âm."))
    except (TypeError, ValueError):
        frappe.throw(_("Ngân sách phải là số."))
    d = getdate(doc.service_month)
    if d.day != 1:
        frappe.throw(_("Tháng dịch vụ (Service month) phải là ngày 1 của tháng."))
