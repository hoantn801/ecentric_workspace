...
"""Permission-safe read + write API for System Request (form #6). Mirrors AI Topup
...
_EDITABLE_DRAFT = ("request_title", "request_type", "other_type", "description", "priority",
...
def get_form_options():
    return {
        "request_types": ["License, account", "Access, permission", "Initiative, solution",
                          "Lark Approvals", "Other"],
...
def list_my_requests(filters=None, start=0, page_length=20):
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "request_type", "priority",
...
def list_fulfillment_queue(section="unclaimed"):
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "requested_by", "request_type", "priority",
                                  "requester_expected_resolution_date", "operation_expected_completion_date",
                                  "fulfillment_status", "fulfillment_owner", "fulfillment_due_at"],
                          order_by="requester_expected_resolution_date asc", limit_page_length=200)
    return {"rows": rows}
...
def get_detail(name):
    ...
    ff = {"status": biz.fulfillment_status, "owner": biz.fulfillment_owner,
          ...
          "operation_note": biz.operation_note}
    return {
...
def submit_request(name):
    from ecentric_workspace.approval_center.system_request import service as svc
