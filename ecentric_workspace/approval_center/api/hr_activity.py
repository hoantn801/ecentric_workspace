...
_EDITABLE_DRAFT = ("request_title", "activity_type", "activity_type_other", "detail", "start_date",
                   "end_date", "participants", "justification", "estimated_budget",
                   "vendor_trainer_partner_info", "request_attachment", "department", "company")
...
def get_form_options():
    return {"activity_types": ["Double day", "Quarterly team bonding", "Holiday and anniversary",
                               "Company trip", "Year-end party", "Medical checkup", "Monthly L&D", "Other"]}
...
def list_my_requests(filters=None, start=0, page_length=20):
    ...
    if f.get("activity_type"):
        flt["activity_type"] = f["activity_type"]
    ...
    rows = frappe.get_all(BIZ, filters=flt,
                          fields=["name", "request_title", "activity_type", "start_date", "end_date",
...
def submit_request(name):
    from ecentric_workspace.approval_center.hr_activity import service as svc
