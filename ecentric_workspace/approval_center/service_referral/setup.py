...
"""Idempotent, System-Manager-only setup for SERVICE_REFERRAL-V1 (Draft): one fixed-participant
level "Referral Review" (Any One, min 1). Also upserts the EC Approval Type catalog card
...
PROCESS_CODE = "SERVICE_REFERRAL-V1"
APPROVAL_TYPE = "SERVICE_REFERRAL"
CATEGORY = "OTHERS"
CARD_TITLE = "Service referral"
DESCRIPTION = "Request for service referral, not applied for Management, Vendor Acquisition, and Service team"
PROC_TITLE = "Service Referral V1"
LEVEL_NAME = "Referral Review"
DEFAULT_APPROVERS = ["linh.ngo@ecentric.vn", "vinh.vu@ecentric.vn"]


def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run Service referral setup."), frappe.PermissionError)
...
@frappe.whitelist()
def setup_service_referral_v1(review_approvers=None, dry_run=1, apply=0):
    _require_sm()
    ...
    _validate_users("Referral Review Approver", rev, rep)
...
@frappe.whitelist()
def validate_service_referral_v1():
