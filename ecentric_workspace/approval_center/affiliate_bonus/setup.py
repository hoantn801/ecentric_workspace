...
"""Idempotent, System-Manager-only setup for AFFILIATE_BONUS_REQUEST-V1 (Draft): sequential
...
PROCESS_CODE = "AFFILIATE_BONUS_REQUEST-V1"
APPROVAL_TYPE = "AFFILIATE_BONUS_REQUEST"
DEFAULT_VINH = ["vinh.vu@ecentric.vn"]
DEFAULT_CEO = ["lam.nguyen@ecentric.vn"]


def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run Affiliate Bonus setup."), frappe.PermissionError)
...
@frappe.whitelist()
def setup_affiliate_bonus_v1(vinh=None, ceo=None, dry_run=1, apply=0):
    _require_sm()
    dry = int(apply or 0) != 1
    rep = {"mode": "dry_run" if dry else "apply", "planned": [], "errors": [], "warnings": [],
           "notes": [], "result": None}
    users = {1: _parse(vinh, DEFAULT_VINH), 2: _parse(ceo, DEFAULT_CEO)}
    _validate("Vinh", users[1], rep)
    ...
    rep["planned"] = ["process %s (Draft), no SLA (v1)" % PROCESS_CODE,
                      "L1 Vinh Review=%s" % users[1], "L2 CEO Review=%s" % users[2]]
    rep["blockers"] = rep["errors"]
    if rep["errors"]:
        rep["result"] = "BLOCKED"
        return rep
...
def _upsert(users):
    ...
    _upsert_level(1, "Vinh Review", users[1])
    _upsert_level(2, "CEO Review", users[2])


@frappe.whitelist()
def validate_affiliate_bonus_v1():
    ...
    if proc:
        ...
        c(names.get(1) == "Vinh Review", "L1 Vinh Review")
