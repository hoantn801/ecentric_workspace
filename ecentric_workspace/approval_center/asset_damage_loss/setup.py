...
"""Idempotent, System-Manager-only setup for ASSET_DAMAGE_LOSS-V1 (Draft):
L1 Operation Review (Any One of the configured operation reviewers), L2 CEO Review. Identities are
config seed args (emails allowed here only); levels use only User participants. No fulfillment; no
SLA. dry-run default; apply=1 required. Never overwrites an Active process."""
import json
...
PROCESS_CODE = "ASSET_DAMAGE_LOSS-V1"
...
def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run Asset Damage/Loss setup."), frappe.PermissionError)
...
@frappe.whitelist()
def setup_asset_damage_loss_v1(operation_reviewers=None, ceo=None, dry_run=1, apply=0):
    _require_sm()
    ...
    _validate("Operation Reviewer", op, rep)
    ...
    rep["planned"] = [
        ...
        "L1 Operation Review (Any One)=%s" % op,
        "L2 CEO Review (Any One)=%s" % ceos,
    ]
    rep["blockers"] = rep["errors"]
    if rep["errors"]:
        rep["result"] = "BLOCKED"
...
def _upsert(op, ceos):
    ...
    proc.title = "Asset Damage or Loss V1"
    ...
    _upsert_level(1, "Operation Review", op)
    _upsert_level(2, "CEO Review", ceos)


@frappe.whitelist()
def validate_asset_damage_loss_v1():
    ...
    if proc:
        ...
        names = {l.level_no: (l.level_name, l.approval_mode) for l in levels}
        c(names.get(1) == ("Operation Review", "Any One"), "L1 Operation Review (Any One)")
        c(names.get(2, (None, None))[0] == "CEO Review", "L2 CEO Review")
        for l in levels:
