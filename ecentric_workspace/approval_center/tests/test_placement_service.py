...
def _draft(tag):
    """Pre-submit request (NO Approval Request yet) - document setup/placement is EDITABLE.
    The signer plan resolves in PREVIEW mode; B1 guarantees preview<->frozen level_no parity."""
    _profile()
...
def _submitted(tag):
    """Sent request (Approval Request exists) - document setup/placement is IMMUTABLE (Issue 4)."""
    req, biz = _draft(tag)
...
class TestPlacementService(FrappeTestCase):
    ...
    def test_1_requester_saves_valid_slot(self):
        req, biz = _draft("p1"); ref = _attach(biz, req)
        frappe.set_user(req)
    ...
    def test_2_invalid_slot_rejected(self):
        req, biz = _draft("p2"); ref = _attach(biz, req)
        frappe.set_user(req)
    ...
    def test_3_non_requester_denied(self):
        req, biz = _draft("p3"); ref = _attach(biz, req)
    ...
    def test_4_admin_and_sm_not_bypassed(self):
        req, biz = _draft("p4"); ref = _attach(biz, req)
    ...
    def test_5_frozen_package_denied(self):
        req, biz = _draft("p5"); ref = _attach(biz, req)
        frappe.set_user(req)
    ...
    def test_6_supporting_document_denied(self):
        req, biz = _draft("p6"); ref = _attach(biz, req)
        frappe.set_user(req)
    ...
    def test_7_non_pdf_denied(self):
        req, biz = _draft("p7"); ref = _attach(biz, req, "pic.png", PNG)
        frappe.set_user(req)
        out = ps.save_placement(BD, biz, ref, _box("requester"))
        frappe.set_user("Administrator")
    ...
    def test_8_document_scoped_preserves_sibling(self):
        req, biz = _draft("p8"); a = _attach(biz, req, "A.pdf", fx.PDF)
    ...
    def test_9_10_progress_unique_covered_not_row_count(self):
        req, biz = _draft("p9"); ref = _attach(biz, req)
        frappe.set_user(req)
    ...
    def test_11_legacy_unmapped_not_counted(self):
        req, biz = _draft("p11"); ref = _attach(biz, req)
        frappe.set_user(req)
        ps.save_placement(BD, biz, ref, _box("requester"))
        draft = pkgsvc.draft_package_for_business(BD, biz)
    ...
    def test_12_delete_updates_progress(self):
        req, biz = _draft("p12"); ref = _attach(biz, req)
        frappe.set_user(req)
        out = ps.save_placement(BD, biz, ref, _box("requester"))
    ...
    def test_13_reload_restores_exact_placement(self):
        req, biz = _draft("p13"); ref = _attach(biz, req)
        frappe.set_user(req)
    ...
    def test_14_15_16_modes_reflected_in_required_slots(self):
        req, biz = _draft("p14"); ref = _attach(biz, req)
    ...
    def test_17_placement_state_zero_writes(self):
        req, biz = _draft("z1"); ref = _attach(biz, req)          # fresh default-signable PDF, no pkg
    ...
    def test_18_first_save_materializes_one_pkg_one_dsf_then_reuses(self):
        req, biz = _draft("z2"); ref = _attach(biz, req)
    ...
    def test_19_delete_never_creates_pkg_or_dsf(self):
        req, biz = _draft("z3"); ref = _attach(biz, req)
    ...
    def test_20_level_no_compat_from_frozen_plan_slot(self):
        req, biz = _draft("z4"); ref = _attach(biz, req)
        plan = sp.resolve_signer_plan(BD, biz)
    ...
    # ---- Issue 4: setup is IMMUTABLE once the request has been sent ('Gửi yêu cầu') ----
    def test_21_presubmit_editable_then_postsubmit_readonly(self):
        req, biz = _draft("s1"); ref = _attach(biz, req)
        frappe.set_user(req)
        self.assertTrue(ps.placement_state(BD, biz, ref)["editable"])   # pre-submit: editable
        ps.save_placement(BD, biz, ref, _box("requester"))              # can place
        frappe.set_user("Administrator"); papi.submit_request(biz)      # -> Gửi yêu cầu
        frappe.set_user(req)
        st = ps.placement_state(BD, biz, ref)
        frappe.set_user("Administrator")
        self.assertFalse(st["editable"])                               # post-submit: view-only
        self.assertEqual(st["setup_editable_reason"], "already_submitted")
        self.assertTrue(st["placements"])                             # existing placements still visible

    def test_22_postsubmit_save_placement_denied(self):
        req, biz = _submitted("s2"); ref = _attach(biz, req)
        frappe.set_user(req)
        with self.assertRaises(frappe.ValidationError):
            ps.save_placement(BD, biz, ref, _box("requester"))         # add denied at backend
        frappe.set_user("Administrator")

    def test_23_postsubmit_delete_placement_denied(self):
        # place while draft, then submit, then deletion must be rejected server-side
        req, biz = _draft("s3"); ref = _attach(biz, req)
        frappe.set_user(req)
        out = ps.save_placement(BD, biz, ref, _box("requester"))
        ...
        frappe.set_user(req)
        with self.assertRaises(frappe.ValidationError):
            ps.delete_placement(BD, biz, ref, pl)                      # delete denied
        frappe.set_user("Administrator")
        self.assertTrue(frappe.db.exists(PL, pl))                     # placement survived (not deleted)

    def test_24_shared_predicate_matches_document_setup(self):
        # the SAME governed predicate drives placement + classification editability
        req, biz = _draft("s4")
        self.assertEqual(ds._setup_editable(BD, biz)[0], True)         # draft -> editable
        frappe.set_user(req); papi.submit_request(biz); frappe.set_user("Administrator")
