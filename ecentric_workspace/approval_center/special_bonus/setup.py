...
"""Idempotent, System-Manager-only setup for SPECIAL_BONUS-V1 (Draft):
...
def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run Special Bonus setup."), frappe.PermissionError)
...
@frappe.whitelist()
def setup_special_bonus_v1(cnb=None, hof=None, ceo=None, dry_run=1, apply=0):
    _require_sm()
...
def validate_special_bonus_v1():
    ...
    if proc:
        ...
        for no, nm in [(1, "Direct Manager Review"), (2, "CnB Review"), (3, "HOF Review"), (4, "CEO Review")]:
