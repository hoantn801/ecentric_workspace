...
PROCESS_CODE = "PAYMENT_REQUEST-V1"
...
def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run Payment Request setup."), frappe.PermissionError)
...
@frappe.whitelist()
def setup_payment_request_v1(finance=None, hof=None, ceo=None, dry_run=1, apply=0):
    _require_sm()
...
@frappe.whitelist()
def validate_payment_request_v1():
