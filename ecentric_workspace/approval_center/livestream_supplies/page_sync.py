...
def _html():
    ...
    with open(os.path.join(base, "frontend", "livestream_supplies.main_section.html"), encoding="utf-8") as fh:
        return fh.read()
...
def sync(html=None):
    ...
    frappe.logger("approval_center").info("livestream_supplies page_sync: %s Web Page /%s" % (action, ROUTE))
