...
"""Idempotent, System-Manager-only setup for LIVESTREAM_SUPPLIES-V1 (Draft): one fixed-participant
level "Sang Bui Review" (Any One, min 1). Also upserts the EC Approval Type catalog card
...
PROCESS_CODE = "LIVESTREAM_SUPPLIES-V1"
APPROVAL_TYPE = "LIVESTREAM_SUPPLIES"
CATEGORY = "OTHERS"
CARD_TITLE = "Livestream supplies request"
DESCRIPTION = "Request for borrowing livestream supplies"
PROC_TITLE = "Livestream Supplies V1"
LEVEL_NAME = "Sang Bui Review"
DEFAULT_APPROVERS = ["sang.bui@ecentric.vn"]


def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run Livestream supplies request setup."), frappe.PermissionError)
...
@frappe.whitelist()
def setup_livestream_supplies_v1(review_approvers=None, dry_run=1, apply=0):
    _require_sm()
    ...
    _validate_users("Sang Bui Review Approver", rev, rep)
...
@frappe.whitelist()
def validate_livestream_supplies_v1():
