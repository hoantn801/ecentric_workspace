...
"""Idempotent, System-Manager-only setup for LATERAL_MOVE-V1 (Draft):
L1 Current Direct Manager Review (Requester Manager, no fallback - blocked at submit if unresolved),
L2 New Line Manager Review (Reference User Field on new_line_manager), L3 HR Review, L4 CEO Review
(each Any One). HR/CEO identities are config seed args (emails allowed here only); L1/L2 are dynamic.
No fulfillment; no SLA. dry-run default; apply=1 required. Never overwrites an Active process."""
import json
...
PROCESS_CODE = "LATERAL_MOVE-V1"
APPROVAL_TYPE = "LATERAL_MOVE"
NLM_FIELD = "new_line_manager"   # business field the Reference User Field resolver reads for L2
...
def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run Lateral Move setup."), frappe.PermissionError)
...
@frappe.whitelist()
def setup_lateral_move_v1(hr=None, ceo=None, dry_run=1, apply=0):
    _require_sm()
    ...
    rep["planned"] = [
        ...
        "L1 Current Direct Manager Review (Requester Manager, Any One, no fallback)",
        "L2 New Line Manager Review (Reference User Field on %s, Any One)" % NLM_FIELD,
        "L3 HR Review (Any One)=%s" % hrs,
        "L4 CEO Review (Any One)=%s" % ceos,
    ]
    rep["blockers"] = rep["errors"]
    if rep["errors"]:
        rep["result"] = "BLOCKED"
...
def _upsert(hrs, ceos):
    ...
    proc.title = "Employee Lateral Move V1"
    ...
    def _upsert_level(no, name, kind, ulist):
        ...
        elif kind == "new_line_manager":
            lvl.append("participants", {"participant_purpose": "Approver",
                                        "source_type": "Reference User Field",
                                        "reference_field": NLM_FIELD, "sort_order": 0})
        else:
    ...
    _upsert_level(1, "Current Direct Manager Review", "manager", None)
    _upsert_level(2, "New Line Manager Review", "new_line_manager", None)
    _upsert_level(3, "HR Review", "user", hrs)
    _upsert_level(4, "CEO Review", "user", ceos)


@frappe.whitelist()
def validate_lateral_move_v1():
    ...
    if proc:
        ...
        for no, nm in [(1, "Current Direct Manager Review"), (2, "New Line Manager Review"),
                       (3, "HR Review"), (4, "CEO Review")]:
        ...
        for l in levels:
            parts = frappe.get_all("EC Approval Participant",
                                   ...
                                   fields=["source_type", "reference_field", "user"])
            if l.level_no == 1:
                c(any(p.source_type == "Requester Manager" for p in parts), "L1 Requester Manager source")
            elif l.level_no == 2:
                c(any(p.source_type == "Reference User Field" and p.reference_field == NLM_FIELD
                      for p in parts), "L2 Reference User Field on %s" % NLM_FIELD)
            else:
