...
"""Idempotent, System-Manager-only setup for SYSTEM_REQUEST-V1 (Draft):
...
PROCESS_CODE = "SYSTEM_REQUEST-V1"
...
def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run System Request setup."), frappe.PermissionError)
...
@frappe.whitelist()
def setup_system_request_v1(review_approvers=None, fulfillers=None, dry_run=1, apply=0):
    _require_sm()
    ...
    _validate_users("Operation Reviewer", rev, rep)
    ...
    rep["planned"] = [
        ...
        "L1 Operation Review (Any One) approvers=%s" % rev,
...
def _upsert_process(rev, ff):
    ...
    lvl.level_no = 1
    lvl.level_name = "Operation Review"
    lvl.mandatory = 1
...
@frappe.whitelist()
def validate_system_request_v1():
    ...
    if proc:
        ...
        c(bool(levels) and levels[0].level_name == "Operation Review" and levels[0].approval_mode == "Any One",
          "L1 Operation Review Any One")
