...
"""Idempotent, System-Manager-only setup for LATE_EARLY_OUT-V1 (Draft): a single level
...
PROCESS_CODE = "LATE_EARLY_OUT-V1"
APPROVAL_TYPE = "LATE_EARLY_OUT"


def _require_sm():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may run Late in - Early out setup."), frappe.PermissionError)


@frappe.whitelist()
def setup_late_early_out_v1(dry_run=1, apply=0):
    _require_sm()
...
def _upsert():
    ...
    proc.title = "Late in - Early out V1"
...
@frappe.whitelist()
def validate_late_early_out_v1():
