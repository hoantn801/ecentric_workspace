...
"""Idempotent sync for the LEGACY /form-rec Web Page (legacy creation form).
...
def sync_form_rec_page():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may sync the /form-rec page."), frappe.PermissionError)
    return sync()
