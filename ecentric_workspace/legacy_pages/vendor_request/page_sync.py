...
"""Idempotent sync for the LEGACY /vendor-request Web Page (document page).
...
def sync_vendor_request_page():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may sync the /vendor-request page."), frappe.PermissionError)
    return sync()
