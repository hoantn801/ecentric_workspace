...
"""Idempotent sync for the LEGACY /so-form Web Page (legacy creation form).
...
def sync_so_form_page():
    if "System Manager" not in frappe.get_roles(frappe.session.user):
        frappe.throw(_("Only System Manager may sync the /so-form page."), frappe.PermissionError)
    return sync()
