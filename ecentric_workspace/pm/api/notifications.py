...
def notify_task_assignment(users, task_name, subject="", actor=None):
    """SYNCHRONOUS task_assigned delivery (called inside the PM assignment transaction, after
    assign_to.add). Routes toast/sound/desktop + Teams via the synchronous web-process service
    publish_task_assignment_delivery -- NO RQ job, NO queue, NO polling, NO native-log wait.
    The native Frappe Assignment log remains the inbox. Skips actor/self, Administrator and
    terminal tasks; idempotent by stable dedupe task_assigned|<task>|<recipient>; fail-open."""
    ...
    for u in users or []:
        ...
        seen.add(u)
        try:
            ncev.publish_task_assignment_delivery(
                u, task_name, subject, action_url=url, actor=actor,
                dedupe_key=_stable_dedupe("task_assigned", task_name, u))
        except Exception:
            frappe.log_error(frappe.get_traceback(), "PM notify_task_assignment")
