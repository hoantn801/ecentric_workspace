...
def _compare():
    """No permission check here - callers decide who may call this (check() gates
    itself for direct/manual use; blockers() is only ever called from paths that
    already gated System Manager, e.g. payment_request.activation.publish_*)."""
    try:
...
def check():
    ...
    for the pair is reported as a mismatch, not silently resolved. Call by hand."""
    _require_sm()
    return _compare()


def blockers():
    """Plain (non-whitelisted) helper for activation-style callers that already gate
    System Manager and already collect a list of blocker strings (see
    payment_request.activation.publish_payment_request_after_uat). Returns [] when
    there is nothing to block on: either the live Profile matches EXPECTED_PROFILE_POLICY,
    or no enabled Profile exists yet (esign is simply not in play for this type -
    absence is not drift)."""
    res = _compare()
    if res["ok"] is not False:
        return []
    if res.get("error"):
        return ["esign flow: %s" % res["error"]]
    return ["esign flow drift on Profile %s: %s = %r (expected %r)"
           % (res["profile"], m["field"], m["live"], m["expected"])
